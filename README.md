## TODO
1. 现在的高箱配额在舱内、甲板都是：对于n层tier，若n>=2, 那么配额= n-1；当n=1，配额=1；
这里需要加上如果是舱内n=4的情况，配额是2，也就是2高+2普
2. 现在高箱有悬空，正在进程【高箱-2】当中诊断，需要仔细理解一下
3. 尾箱处理有bug，分析完之后看看能配进去多少箱子；现在运行会有assertion error
4. 20ft的cbf成功生成了，完成了只拆分40ft，保存数据合理；下一步是给有些单数小箱打补丁装到only20去；第三步再考虑利用bay 01，加这个搜索前的分配；
